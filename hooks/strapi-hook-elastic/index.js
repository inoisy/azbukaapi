...
class ElasticsearchAdapter extends AbstractNosqlAdapter {
  ...
  constructor(app_config) {
    ...
    console.log('Elasticsearch module initialized!');
    this.updateDocument.bind(this);
  }
  ...
  searchDocuments(collectionName, queryBody) {
    return new Promise((resolve, reject) => {
      this.db.search({ // requires ES 5.5
        index: this.config.db.indexName,
        type: collectionName,
  ...
  /**
   * Create single document in database
   * @param {object} item document to be updated in database
   */
  createDocument(collectionName, item) {

    const itemtbu = item;
    // this.db.cluster.putSettings({
    //   body: {
    //     index: {
    //       blocks: {
    //         bulk: {
  ...
  updateDocumentBulk(collectionName, items) {
    ...
    for (let doc of items) {
      ...
      if ((index % bulkSize) == 0) {
        console.log('Splitting bulk query ' + index);
        this.db.bulk({
          body: requests
        }, function (error, response) {
          if (error)
...
module.exports = strapi => {
  const hook = {
    ...
    defaults: {
      ...
      db: {
        url: 'http://localhost:9200',
        indexName: "productfinal",
        type: "doc"
      },
    },
    // async updateDocument(index, item) {
    //   return await this.hook.elastic.updateDocument(index, item)
    // },
    /**
     * Initialize the hook
     */
    ...
    initialize: cb => {
      ...
      console.log(hook.defaults.db.indexName)
      hook.elastic = new ElasticsearchAdapter({
        db: {
          url: hook.defaults.db.url,
          indexName: hook.defaults.db.indexName
        }
      });
      hook.elastic.connect((done) => console.log("done"))
      cb();
    }
  };
